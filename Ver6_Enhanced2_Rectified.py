...
# Market Status (note: US hours approximation)
now = datetime.utcnow()
market_open = now.hour >= 13 and now.hour < 20
if market_open:
    st.sidebar.success("🟢 US Market Open (approx. EST hours shown in UTC)")
else:
    st.sidebar.error("🔴 US Market Closed (approx. EST hours shown in UTC)")
...
for ticker in [t.strip().upper() for t in tickers.split(",")]:
    ...
    # Show configurable crossover result
    ...
    # Also show quick 50/200 check for convenience
